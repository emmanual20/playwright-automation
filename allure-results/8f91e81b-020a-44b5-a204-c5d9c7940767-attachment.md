...
```
Error: locator.innerText: Error: strict mode violation: locator('//*[@id="largeTableBody"]/tr').first().locator('td') resolved to 5 elements:
...
Call log:
  - waiting for locator('//*[@id="largeTableBody"]/tr').first().locator('td')
...
```ts
  ...
  20 |         
  21 |         const count = await this.tableData.count()
  22 |         for (let i = 0; i < count; i++) {
  23 |             const row = this.tableData.nth(i)
> 24 |             const text = await row.locator('td').innerText()
     |                                                  ^ Error: locator.innerText: Error: strict mode violation: locator('//*[@id="largeTableBody"]/tr').first().locator('td') resolved to 5 elements:
